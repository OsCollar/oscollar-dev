...
Dialog(key kID, string sPrompt, list lChoices, list lUtilityButtons, integer iPage, integer iAuth, string sName) {
    ...
    if (iIndex != -1) //we've alread given a menu to this user.  overwrite their entry
...
UserCommand(integer iNum, string sStr, key kID, integer fromMenu) {
    ...
    } else if (sCmd == "lock" || (g_iLocked == FALSE && sStr == "togglelock")) {    //does anything use togglelock?  If not, it'd be nice to get rid of it
    ...
    } else if (llSubStringIndex(sStr,".- ... -.-") == 0) {
        if (kID == g_kWearer) {
...
default {
    ...
    timer() {
        if (g_iWaitUpdate) {
            g_iWaitUpdate = FALSE;
            llListenRemove(g_iUpdateHandle);
            if (g_iWillingUpdaters == FALSE) {   //if no updaters responded, get upgrader info from remenu
